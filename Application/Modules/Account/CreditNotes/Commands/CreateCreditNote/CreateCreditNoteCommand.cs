...
public record CreateCreditNoteResponse(int Id, string NoteNumber);

[AuthorizePermission(Permissions.Accounting.AddCreditNote)]
public record CreateCreditNoteCommand : IRequest<Result<CreateCreditNoteResponse>>
{
    ...
    /// <summary>Required when no SalesInvoiceId is supplied.</summary>
    public Guid? DebitAccountId { get; init; }
    /// <summary>Required when no SalesInvoiceId is supplied.</summary>
    public Guid? CreditAccountId { get; init; }
    /// <summary>Optional link to a source Sales Invoice. When supplied and Items is empty, items are copied from the invoice.</summary>
    ...
    public List<CreateCreditNoteItemRequest> Items { get; init; } = new();
}

public class CreateCreditNoteCommandHandler : IRequestHandler<CreateCreditNoteCommand, Result<CreateCreditNoteResponse>>
{
    ...
    public async Task<Result<CreateCreditNoteResponse>> Handle(CreateCreditNoteCommand request, CancellationToken cancellationToken)
    {
        Guid debitAccountId;
        Guid creditAccountId;
        List<CreateCreditNoteItemRequest> items = request.Items.ToList();

        // Resolve GL accounts and optionally copy items from the linked Sales Invoice
        if (request.SalesInvoiceId.HasValue)
        {
            var si = await _context.GetSet<Novologs.Domain.Entities.SalesInvoice>()
                .Include(i => i.Items)
                .FirstOrDefaultAsync(i => i.Id == request.SalesInvoiceId.Value && !i.IsDeleted, cancellationToken);

            if (si is null)
                return Result<CreateCreditNoteResponse>.Failure("CN_404_SALES_INVOICE_NOT_FOUND",
                    $"Sales invoice with ID {request.SalesInvoiceId.Value} was not found.");

            // Auto-reverse GL accounts from SI when not explicitly provided
            debitAccountId  = request.DebitAccountId  ?? si.CreditAccountId;
            creditAccountId = request.CreditAccountId ?? si.DebitAccountId;

            // Auto-copy items from SI when none supplied
            if (items.Count == 0)
            {
                items = si.Items.Select(i => new CreateCreditNoteItemRequest
                {
        ...
        else
        {
            ...
            if (!request.DebitAccountId.HasValue || request.DebitAccountId.Value == Guid.Empty)
                return Result<CreateCreditNoteResponse>.Failure("CN_400_DEBIT_ACCOUNT_REQUIRED",
                    "DebitAccountId is required when no SalesInvoiceId is supplied.");

            if (!request.CreditAccountId.HasValue || request.CreditAccountId.Value == Guid.Empty)
                return Result<CreateCreditNoteResponse>.Failure("CN_400_CREDIT_ACCOUNT_REQUIRED",
                    "CreditAccountId is required when no SalesInvoiceId is supplied.");
        ...
        if (items.Count == 0)
            return Result<CreateCreditNoteResponse>.Failure("CN_400_NO_ITEMS",
        ...
        if (!debitAccountExists)
            return Result<CreateCreditNoteResponse>.Failure("CN_404_DEBIT_ACCOUNT_NOT_FOUND",
        ...
        if (!creditAccountExists)
            return Result<CreateCreditNoteResponse>.Failure("CN_404_CREDIT_ACCOUNT_NOT_FOUND",
                $"Credit account with ID {creditAccountId} was not found.");

        if (debitAccountId == creditAccountId)
            return Result<CreateCreditNoteResponse>.Failure("CN_400_SAME_ACCOUNTS",
        ...
        if (missingProductId != default)
            return Result<CreateCreditNoteResponse>.Failure("CN_404_PRODUCT_NOT_FOUND",
        ...
        if (inactiveProduct != null)
            return Result<CreateCreditNoteResponse>.Failure("CN_400_PRODUCT_INACTIVE",
                $"Product '{inactiveProduct.Name.Value}' is inactive and cannot be used in a credit note.");
        ...
        for (var idx = 0; idx < calc.Items.Count; idx++)
        {
            ...
            if (r.LineDiscountAmount > r.LineBase)
                return Result<CreateCreditNoteResponse>.Failure("CN_400_LINE_DISCOUNT_EXCEEDS",
        ...
        if (calc.OverallDiscountAmount > calc.SubtotalAfterLineDiscounts)
            return Result<CreateCreditNoteResponse>.Failure("CN_400_OVERALL_DISCOUNT_EXCEEDS",
        ...
        var note = new Novologs.Domain.Entities.CreditNote(0)
        {
            ...
            {
                var r = calc.Items[idx];
                return new Novologs.Domain.Entities.CreditNoteItem
                {
        ...
        return Result<CreateCreditNoteResponse>.Success(new CreateCreditNoteResponse(id, noteNumber));
    }
    ...
    private async Task<string> GenerateNoteNumberAsync(CancellationToken cancellationToken)
    {
        ...
        var existing = await _context.GetSet<Novologs.Domain.Entities.CreditNote>()
