...
namespace Novologs.Application.Modules.Account.CreditNotes.Commands.UpdateCreditNote;

public class UpdateCreditNoteItemRequest
{
...
[AuthorizePermission(Permissions.Accounting.UpdateCreditNote)]
public record UpdateCreditNoteCommand : IRequest<Result<CreditNoteDto>>
{
    ...
    public List<UpdateCreditNoteItemRequest> Items { get; init; } = new();
}
...
public class UpdateCreditNoteCommandHandler : IRequestHandler<UpdateCreditNoteCommand, Result<CreditNoteDto>>
{
    ...
    public async Task<Result<CreditNoteDto>> Handle(UpdateCreditNoteCommand request, CancellationToken cancellationToken)
    {
        ...
        if (note.Status != Novologs.Domain.Enums.CreditNoteStatus.Draft)
            return Result<CreditNoteDto>.Failure("CN_409_NOT_DRAFT", "Only Draft credit notes can be updated.");

        if (request.DebitAccountId == request.CreditAccountId)
            return Result<CreditNoteDto>.Failure("CN_400_SAME_ACCOUNTS", "Debit and credit accounts must be different.");
        ...
        if (missingProductId != default)
            return Result<CreditNoteDto>.Failure("CN_404_PRODUCT_NOT_FOUND",
        ...
        if (inactiveProduct != null)
            return Result<CreditNoteDto>.Failure("CN_400_PRODUCT_INACTIVE",
        ...
        for (var idx = 0; idx < calc.Items.Count; idx++)
        {
            ...
            if (r.LineDiscountAmount > r.LineBase)
                return Result<CreditNoteDto>.Failure("CN_400_LINE_DISCOUNT_EXCEEDS",
        ...
        if (calc.OverallDiscountAmount > calc.SubtotalAfterLineDiscounts)
            return Result<CreditNoteDto>.Failure("CN_400_OVERALL_DISCOUNT_EXCEEDS",
        ...
        note.SalesInvoiceId              = request.SalesInvoiceId;
        ...
        {
            var r = calc.Items[idx];
            note.Items.Add(new Novologs.Domain.Entities.CreditNoteItem
            {
