...
public class DeleteContactCommandAuthorizer : AuthorizationHandler<ZetaAuthorizationRequirement<DeleteContactCommand>>
{
    ...
    {
        var hasPermission = await context.User.HasPermission(_userManager, _context, Novologs.Domain.Constants.Permissions.Clients.DeleteContact);
        if (! hasPermission)
        {
            context.Fail(new AuthorizationFailureReason(this, "User is not authorized to delete contact."));
            return;
        }
        else
        {
