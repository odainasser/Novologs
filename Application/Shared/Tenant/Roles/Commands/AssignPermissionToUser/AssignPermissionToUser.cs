...
public record AssignPermissionToUserCommand : IRequest<Result<AssignPermissionToUserResponse>>
{
    public List<Guid> UserIds { get; set; } = new();
    public List<Guid> PermissionIds { get; set; } = new();
}

public class AssignPermissionToUserResponse
{
}
...
public class
    AssignPermissionToUserCommandHandler : IRequestHandler<AssignPermissionToUserCommand,
    Result<AssignPermissionToUserResponse>>
{
    ...
    public async Task<Result<AssignPermissionToUserResponse>> Handle(AssignPermissionToUserCommand request,
    ...
    { 
        ...
        foreach (var userId in request.UserIds)
        {
            ...
            if (user == null)
            {
                return Result<AssignPermissionToUserResponse>.Failure("User.NotFound",
            ...
            _context.GetSet<UserPermission>().RemoveRange(existingUserPermissions);

            foreach (var permissionId in request.PermissionIds)
            {
                var userPermission = new UserPermission(Guid.NewGuid())
                {
                    UserId = userId, PermissionId = permissionId
                };
                _context.GetSet<UserPermission>().Add(userPermission);
            }
        }

        await _context.SaveChangesAsync(cancellationToken);

        return Result<AssignPermissionToUserResponse>.Success(new AssignPermissionToUserResponse());
    }
}
