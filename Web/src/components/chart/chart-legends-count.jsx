...
export function ChartLegendsCount({
  icons,
  values,
  sublabels,
  labels = [],
  colors = [],
  chartSeries = [], // Make sure to pass chartSeries here
  ...other
}) {
  return (
      ...
      {labels?.map((series, index) => (
        <Stack key={series} spacing={1}>
          <StyledLegend>
            ...
            {chartSeries && chartSeries[index] !== undefined && (
              <Box sx={{ typography: 'body2', fontSize: '1rem', fontWeight: 'bold' }}>
                {`${chartSeries[index]}`}
              </Box>
            )}
            <Box component="span" sx={{ flexShrink: 0, fontSize: '0.875rem' }}>
              {series}
