...
export function LocalPurchaseOrderToolbar({
  filters,
  sharedUsers,
  dateError,
  onResetPage,
  ...
  const handleVendors = useCallback(
    (newValue) => {
      onResetPage();
      filters.setState({ vendor: newValue || null });
    },
    [filters, onResetPage]
  );
    ...
    <>
      <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 260 } }}>
        <Autocomplete
          ...
          options={sharedUsers || []}
          value={filters.state.vendor || null}
          onChange={(event, newValue) => {
            handleVendors(newValue);
          }}
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label={isClientView ? 'Clients' : 'Vendors'}
              placeholder={isClientView ? 'Clients' : 'Vendors'}
              variant="outlined"
              sx={{
          ...
          renderOption={(props, option) => (
            <li {...props} key={option.id}>
              ...
              >
                {!option?.profileImageFileUrl && option?.name?.charAt(0).toUpperCase()}
              </Avatar>
              {option.name}
            </li>
          )}
