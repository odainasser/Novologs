...
export function ProductsView() {
  ...
  const { productList, productListLoading, productListError, productListEmpty, mutate } =
    getProducts();
  ...
  const handleRemoveItem = async (itemToRemove) => {
    if (itemToRemove?.id) {
      try {
        const response = await deleteProduct(itemToRemove.id);
        if (response.success) {
          await mutate();
  ...
  const handleUpdateItem = async () => {
    ...
    const item = productList.products.find((c) => c.id === editingItemId);
    if (!item) return;
  ...
  if (productListError) {
    return <ErrorView errorCode={productListError} />;
  }
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <>
            ...
            <TextField
              ...
              value={newItemAr}
              onChange={(e) => setNewItemAr(e.target.value)}
              placeholder="New Product in Arabic"
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  editingItemId ? handleUpdateItem() : handleAddItem();
                }
