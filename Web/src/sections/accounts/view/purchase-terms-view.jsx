...
export function PurchaseTermsView() {
  const { t, i18n } = useTranslation('dashboard/tasks');
  const storedLang = localStorage.getItem('selectedLang');

  const { termsList, termsListLoading, termsListError, termsListEmpty, mutate } =
    getPurchaseTerms();
  ...
  const handleAddItem = async () => {
    if (newItem.trim()) {
      ...
      try {
        const response = await addPurchaseTerms(payload);
        console.log('this is the response', response);
        if (response.success) {
          await mutate();
          toast.success('Purchase Terms added successfully');
        } else {
          toast.error(response.error);
        }
      } catch (error) {
        console.error('Add purchase terms failed:', error);
      }
  ...
  const handleRemoveItem = async (itemToRemove) => {
    if (itemToRemove?.id) {
      try {
        const response = await deletePurchaseTerms(itemToRemove.id);
        if (response.success) {
          await mutate();
          toast.success('Purchase Terms deleted successfully');
        } else {
          toast.error(response.error);
        }
      } catch (error) {
  ...
  const handleUpdateItem = async () => {
    ...
    const item = termsList.terms.find((c) => c.id === editingItemId);
    if (!item) return;
    if (newItem.trim() && editingItemId) {
      ...
      try {
        const response = await updatePurchaseTerms(payload);
        if (response.success) {
          await mutate();
          toast.success('Purchase Terms updated successfully');
        ...
        console.error('Update purchase terms type failed:', error);
      }
    }
  };
  if (termsListLoading)
  ...
  if (termsListError) {
    return <ErrorView errorCode={termsListError} />;
  }
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <>
            <TextField
              ...
              value={newItem}
              onChange={(e) => setNewItem(e.target.value)}
              placeholder="New Purchase Terms"
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
            ...
            <TextField
              ...
              value={newItemAr}
              onChange={(e) => setNewItemAr(e.target.value)}
              placeholder="New Purchase Terms in Arabic"
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  editingItemId ? handleUpdateItem() : handleAddItem();
                }
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mt: 1 }}>
        <Stack direction="row" spacing={1}>
          <ConfirmDialog
            ...
            content="Are you sure you want to delete this Purchase Terms?"
            action={
              <Button
                variant="contained"
                color="error"
