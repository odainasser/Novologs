...
export function PurchaseUnitsView() {
  const { t, i18n } = useTranslation('dashboard/tasks');
  const storedLang = localStorage.getItem('selectedLang');

  const { productUnitList, productUnitListLoading, productUnitListError, productUnitListEmpty, mutate } =
    getProductUnits();
  ...
  const handleAddItem = async () => {
    if (newItem.trim()) {
      ...
      try {
        const response = await addProductUnits(payload);
  ...
  const handleRemoveItem = async (itemToRemove) => {
    if (itemToRemove?.id) {
      try {
        const response = await deleteProductUnit(itemToRemove.id);
        if (response.success) {
          await mutate();
  ...
  const handleUpdateItem = async () => {
    ...
    const item = productUnitList.units.find((c) => c.id === editingItemId);
    if (!item) return;
    if (newItem.trim() && editingItemId) {
      ...
      try {
        const response = await updateProductUnit(payload);
        if (response.success) {
          await mutate();
        ...
        console.error('Update unit type failed:', error);
      }
    }
  };
  if (productUnitListLoading)
    return (
      <div>
        <LinearProgress
          sx={{
  ...
  if (productUnitListError) {
    return <ErrorView errorCode={productUnitListError} />;
  }
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <>
            ...
            <TextField
              ...
              value={newItemAr}
              onChange={(e) => setNewItemAr(e.target.value)}
              placeholder="New Unit in Arabic"
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  editingItemId ? handleUpdateItem() : handleAddItem();
                }
