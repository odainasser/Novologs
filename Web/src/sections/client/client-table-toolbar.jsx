...
export function ClientTableToolbar({
  ...
  const handleFilterMyClient = useCallback(
    (event) => {
      const newValue =
        typeof event.target.value === 'string' ? event.target.value.split(',') : event.target.value;
  ...
  return (
    <>
      ...
      >
        ...
        />
        {isClientView && zetaUser?.permissions?.includes('Client.ViewAll') && (
          <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 200 } }}>
            <InputLabel htmlFor="client-filter-myClient-select-label" sx={{ top: '-6px' }}>
              {t('clients.labels.all_clients')}
            </InputLabel>
            <Select
              value={filters.state.myClient}
              onChange={handleFilterMyClient}
              input={<OutlinedInput label={t('clients.columns.myClient')} />}
              renderValue={(selected) => selected.map((value) => value).join(', ')}
              inputProps={{ id: 'client-filter-myClient-select-label' }}
              sx={{
                '& .MuiInputBase-input': {
                  padding: '10px 14px',
                },
        ...
        {!isClientView && zetaUser?.permissions?.includes('Vendor.ViewAll') && (
          <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 200 } }}>
            <InputLabel htmlFor="client-filter-myClient-select-label" sx={{ top: '-6px' }}>
              {t('clients.labels.all_vendors')}
            </InputLabel>
            <Select
              // multiple
              value={filters.state.myClient}
        ...
        <FormControl sx={{ flexShrink: 0, width: { xs: 1, md: 200 } }}>
          <InputLabel htmlFor="user-filter-member-select-label" sx={{ top: '-6px' }}>
            {t('clients.columns.members')}
          </InputLabel>
          <Select
            multiple
            value={filters.state.members}
            onChange={handleFilterMembers}
            input={<OutlinedInput label={t('clients.columns.members')} />}
            renderValue={(selected) => selected.map((value) => value).join(', ')}
