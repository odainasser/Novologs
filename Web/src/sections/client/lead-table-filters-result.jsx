...
export function LeadTableFiltersResult({ filters, onResetPage, totalResults, sx }) {
  ...
  const handleRemoveMyLead = useCallback(
    (inputValue) => {
      const newValue = filters.state.myLead.filter((item) => item !== inputValue);
  ...
  const handleReset = useCallback(() => {
    ...
    filters.setState({ name: '', status: [], myLead: [], members: [] });
  }, [filters, onResetPage]);
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={handleReset} sx={sx}>
      ...
      <FiltersBlock label="Leads:" isShow={!!filters.state.myLead.length}>
        {filters.state.myLead.map((item) => (
          <Chip {...chipProps} key={item} label={item} onDelete={() => handleRemoveMyLead(item)} />
        ))}
      </FiltersBlock>
