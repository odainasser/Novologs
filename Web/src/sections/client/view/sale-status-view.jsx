...
export function SaleStatusView() {
  ...
  const { salesStatusList, salesStatusListEmpty, mutate } = getSaleStatuses();
  ...
  const handleAddStatus = async () => {
    if (newStatus.trim()) {
      ...
      try {
        const response = await addSaleStatus(payload);
        if (response.success) {
          await mutate();
          toast.success(t('clients.toast.sales_status_added'));
        } else {
  ...
  const handleRemoveStatus = async (statusToRemove) => {
    if (statusToRemove?.id) {
      try {
        const response = await deleteSaleStatus(statusToRemove.id);
        if (response.success) {
          await mutate();
          toast.success(t('clients.toast.sales_status_deleted'));
        } else {
          toast.error(response.error);
        }
      } catch (error) {
  ...
  const handleUpdateStatus = async () => {
    ...
    const status = salesStatusList.status.find((s) => s.id === editingStatusId);
    if (!status) return;
    if (newStatus.trim() && editingStatusId) {
      ...
      try {
        const response = await updateSaleStatus(payload);
        if (response.success) {
          await mutate();
          toast.success(t('clients.toast.sales_status_updated'));
        ...
        <Stack direction="row" spacing={2}>
          <>
            <TextField
              ...
              value={newStatus}
              onChange={(e) => setNewStatus(e.target.value)}
              placeholder={t('clients.labels.sales_status')}
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
            ...
            <TextField
              ...
              value={newStatusAr}
              onChange={(e) => setNewStatusAr(e.target.value)}
              placeholder={t('clients.labels.sales_status_arabic')}
              onKeyPress={(e) => {
                if (e.key === 'Enter') {
                  editingStatusId ? handleUpdateStatus() : handleAddStatus();
                }
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mt: 1 }}>
        <Stack direction="row" spacing={1}>
          <ConfirmDialog
            ...
            content={t('leaddetails.confirm.confirm_status')}
            action={
              <Button
                variant="contained"
                color="error"
