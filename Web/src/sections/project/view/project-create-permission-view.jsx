...
export function ProjectCreatePermissionView() {
  ...
  const handleExemptEmployee = async (employeesToRemove) => {
    if (employeesToRemove.id) {
      toast.success(t('projects.toast.employee_exempted'));
    }
  };
  ...
  const addNewGroup = (newGroup) => {
    ...
    toast.success(t('projects.toast.permission_add'));
    setSelectedPersons([]);
  };
  ...
  return (
    <Stack spacing={1} sx={{ p: 3, pt: 1 }}>
      <Box display="flex" justifyContent="end">
        ...
        >
         {t('projects.toast.add_employees')}
        </Button>
        <AddMemberDetails
          open={members}
          shared={_members}
