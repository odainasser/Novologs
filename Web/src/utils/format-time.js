...
export function fDateTimeNew(date, format = 'DD-MM-YYYY, hh:mm A') {
  if (!date) {
    return null;
  }
...
export function fToNow(date) {
  ...
  const parsed = dayjs.utc(date).local(); // 👈 FIX

  return parsed.isValid() ? parsed.toNow(true) : 'Invalid time value';
}
